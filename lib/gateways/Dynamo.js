...
module.exports = (config) => {
  const Dynamo = {
    createGroup: async (group) => {
      ...
      for(let i=0; i<100; i++){
        try{
          ...
          const Item = { groupName: group.name, metricValues, metricUnits, id: group.id, groupKeys: {} };
    ...
    createGroupKey: async (id, keyData) => {
      const params = {
        TableName: config.tables.groupsTable,
        Key: { id },
    ...
    listGroupKeys: async (id) => {
      const group = await config.client.aget({
        TableName: config.tables.groupsTable,
        Key: { id }
      });
      if(group.Item) return Object.values(group.Item.groupKeys);
    },

    deleteGroupKey: async (groupId, keyId) => {
      await config.client.aupdate({
        TableName: config.tables.groupsTable,
        Key: { id: groupId },
    ...
    getMetric: async (groupId, metricName, options) => {
